...
import {isNode, isToken, isNOT, isExpression, Node, Token, Predicate, isPredicate} from '../b-tree';
import {weight} from '../b-tree/operations';
...
test('Parser', (t: Test) => {
  ...
  t.equal(weight(parse('lung AND cancer') as Node), 3, 'The weight of "lung AND cancer" is 3');
  t.ok(isExpression(parse('lung cancer')), 'The result of "lung cancer" is an expression');
  t.equal(weight(parse('lung cancer') as Node), 3, 'The weight of "lung cancer" is 3');
  t.ok(isExpression(parse('lung OR cancer')), 'The result of "lung OR cancer" is an expression');
  t.equal(weight(parse('lung OR cancer') as Node), 3, 'The weight of "lung OR cancer" is 3');

  // Nesting
  t.equal(weight(parse('this (lung OR cancer)') as Node), 5, 'The weight of "this (lung  OR cancer) is 5"');
